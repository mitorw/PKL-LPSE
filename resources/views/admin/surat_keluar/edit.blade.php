...
<form action="{{ route('surat_keluar.update', $surat->id) }}" method="POST" enctype="multipart/form-data">
    ...
    @method('PUT')
        ...
        <div class="mb-3">
            <label>Tanggal</label>
            <input type="date" name="tanggal" class="form-control" value="{{ old('tanggal', $surat->tanggal) }}" required>
        </div>

        <div class="mb-3">
            <label>Dibuat Oleh</label>
            <input type="text" name="dibuat_oleh" class="form-control" value="{{ old('dibuat_oleh', $surat->dibuat_oleh) }}" required>
        </div>
